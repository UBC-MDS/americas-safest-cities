  ...
  tabsetPanel(
    ...
    tabPanel("Interactive Map",
             fluidRow(
               sidebarPanel(
                 width = 3,
                 helpText("Click on a city's bubble to view detials"),
                 # sliderInput(
                 #   "yearInput",
                 #   "Year",
                 #   min = 1975,
             ...
             fluidRow(
               sidebarLayout(
                 sidebarPanel(
                   ...
                   pickerInput(
                     inputId = "cityInput",
                     label = "Select up to 5 cities to compare",
...
server <- function(input, output) {
  output$map <- renderLeaflet({
    ...
    leaf_data <- data %>%
      ...
      mutate(rank = dense_rank(desc(Numbers)))
